...
using AdsTest.Migrations.SeedData;
...
namespace AdsTest.Tests
{
    public abstract class AdsTestTestBase : AbpIntegratedTestBase<AdsTestTestModule>
    {
        ...
        protected void UsingDbContext(Action<AdsTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AdsTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AdsTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AdsTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AdsTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AdsTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AdsTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AdsTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AdsTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AdsTestDbContext, Task<T>> func)
        {
            T result;
